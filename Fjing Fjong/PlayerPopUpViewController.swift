...
class PlayerPopUpViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for player in players {
            ...
            playerItems.append(SearchTextFieldItem(title: player.name!, subtitle: String(format:"%.2f", player.rating), image: UIImage(data: imageData!)))
        }
    ...
    @IBAction func ConfirmButtonPressed(_ sender: Any) {
        ...
        for player in players {
            if(playerOneSearchTextField.text!.lowercased() == player.name?.lowercased()){
                appDelegate.playerOneGUID = player.id!
            }
            if(playerTwoSearchTextField.text!.lowercased() == player.name?.lowercased()){
                appDelegate.playerTwoGUID = player.id!
            }
            if(playerThreeSearchTextField.text!.lowercased() == player.name?.lowercased()){
                appDelegate.playerThreeGUID = player.id!
            }
            if(playerFourSearchTextField.text!.lowercased() == player.name?.lowercased()){
                appDelegate.playerFourGUID = player.id!
            }
        }
        
        print(appDelegate.playerOneGUID)
        print(appDelegate.playerTwoGUID)
        print(appDelegate.playerThreeGUID)
        print(appDelegate.playerFourGUID)
        
        
        if (appDelegate.willStream){
            let TabBarViewController = self.presentingViewController as! TabBarController
            let PlayViewController = TabBarViewController.selectedViewController as! PlayController
            PlayViewController.startBroadcast()
